...
在这个例子中，我们在子组件中定义了一个名为 onCustomEvent 的 props 属性，用于接收父组件传递过来的函数。然后，
在需要触发自定义事件的地方调用该函数，并传递一个包含 message 字段的对象作为参数。
...
在这个例子中，我们在 jsonData 中为每一个子组件添加了一个名为 onCustomEvent 的属性，用于定义该子组件的自定义事件。其中，type 
属性表示自定义事件的类型，payload 属性用于存储自定义事件的数据（可以为空对象 {}）。
