...
在类组件中，我们可以使用 shouldComponentUpdate() 或 PureComponent，这些很容易理解和实现，但这些技术在函数组件中并不适用。为了解决这个问题，React Hooks 提供了两个 Hook：React.memo() 和 useCallback()，同时还有一个 useMemo()。

React.memo()
React.memo() 是一个高阶组件（higher-order component），它类似于 PureComponent，但是它用于函数组件而不是类组件。当组件的 props 发生更改时，React.memo() 会比较前后两个 props 是否相等。如果 props 相等，则不会重新渲染组件。
...
useCallback()
useCallback() 是一个 Hook，它可以在组件重新渲染时缓存一个函数回调。这意味着我们可以控制什么时候创建一个新的函数回调。如果你的组件需要传递一个回调函数给子组件，而这个回调函数的代码在父组件中会重新生成，这时使用 useCallback() 可以避免子组件不必要地重新渲染。
