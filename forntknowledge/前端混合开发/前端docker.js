...
这个配置文件指定了 Nginx 监听 80 端口，并将前端项目的域名或 IP 地址设置为 frontend.example.com。然后，将前端项目的静态文件目录设置为 Nginx 默认网站的根目录。接着，设置 HTTP 请求头的 Cache-Control 为 no-cache，以确保前端页面能够实时更新。当出现 404 错误时，跳转到自定义的 404 页面 /404.html。最后，使用 try_files 指令将所有非 API 请求（即没有 /api 路径）的 URL 重写到 /index.html，以支持前端路由。
