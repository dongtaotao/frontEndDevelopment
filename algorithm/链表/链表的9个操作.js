...
它们都能够在 O(1) 时间内删除第一个结点。它们都能够在 O(1) 时间内在给定结点之后或列表开头添加一个新结点。它们都无法在常量时间内随机访问数据。
但是删除给定结点(包括最后一个结点)时略有不同。

在单链表中，它无法获取给定结点的前一个结点，因此在删除给定结点之前我们必须花费 O(N) 时间来找出前一结点。在双链表中，这会更容易，因为我们可以使用“prev”引用字段获取前一个结点。因此我们可以在 O(1) 时间内删除给定结点。
...
递归出口：任意一个链表为空时,直接return 另外一个链接，也就是拼接过程从两个链表中依次取出节点比较，小的那一个就拎出来作为下一个链表节点
代码点这里☑️
...
每次把当前curr指针指向上一个prenext保存下一个节点信息
...
因为是k个分组，所以得有一个count计数，记录节点个数。start指针代表的含义就是start记录的信息是当前分组的起始节点位置的前一个节点。end指针代表的含义就是要区间翻转的后一个节点。翻转后，start指向翻转后链表, 区间（start，end）中的最后一个节点, 返回start 节点。此时还需要将翻转后的分组中最后一个节点指向下一个分组，也就是front.next = cur也就是图中值为1节点指向end
...
建立一个newNode对链表进行k个单位分组，记录每一组的起始和最后节点位置对每一组进行相应的翻转，记得更换位置返回newNode.next
...
设置两个指针,每条指针走完自己的路后,指向另外一个链表,那么两个节点相等的话，一定是同一个点。因为两个指针走的距离是一样的,而且每次都前进1，距离相等,速度相同,如果相等，一定是同一个点。
