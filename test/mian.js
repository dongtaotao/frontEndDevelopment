...
面试官你好，我叫董涛，我目前在上海爱奇艺基础平台架构组，在自爱奇艺已经三年多了，
目前主要负责公司性能监控分析平台网站的建设，
负责项目整体技术架构以及研发工作，  比如整体框架搭建，技术方案制定，性能优化相，制定开发规范，推动前端工程化等
带领一个小团队参与项目开发，--------是项目负责人。
同时也维护公司JSSDK技术框架，以及参与公司运营后台，以及H5相关的开发开发。
前期参与过公司React-Native 框架建设，维护以及业务开发。 (Js 部分以及 Native 与 Js 通信的桥接和 CICD)
也参公司Flutter框架的建设，落地公司第一个Flutter项目。
个人对前端，web，H5，Hybrid，以及跨平台端等技术有着丰富的开发经验。
主要技术站是有react，vue，小程序，RN，flutter的等。
平时喜欢调研一些新技术，看一些技术文章，            ---------每周会把看到不错的文章写在周报里分享给同事
平时也会写技术博客，总结学习（目前个人维护的 CSDN 技术博客访问量已经达到一千多万条了，自己写了三百多篇文章）。
上一家公司是在上海京东。在京东业主要从事前端相关的项目开发

1.项目背景
    公司内部有几十个app，但是每个app的质量我们没法保证，也没有统一的监控。每个业务线app投递的数据（比如用户行为数据，性能数据，异常数据），可能是第三方网站，也可能是自己内部开发分析平台。
    总之无法统一收集与管理,保证，多有app的质量。
...
🔥🔥🔥🔥🔥🔥🔥🔥负责技术选型，项目框架搭建，集成框架React全家桶，让项目更加模块化，组件化，规范化，自动化。 
  🌰技术选型选什么：技术没有好坏之分，看是否适合团队 前端框架（vue， react， angular node.js等） 语言（js，ts）其他（构建工具，CI/CD 等）
    技术选型的依据
      1.要站在团队的角度，而非个人的角度。2.团队成员的学习成本3.是否符合团队的技术栈
      4.公司是否已经有经验积累，社区是否已经足够成熟 5.是否符合项目需求----   
        ---前端框架选型（vue，react，angular）状态管理，构建工具选型（vite，webpack roolup），
        自动化选型（jenkins， gitlabci， github action）
  ...
  🌰为什么要搭建脚手架。https://blog.51cto.com/u_3044148/3351023
    平时在创建react项目时，我们总喜欢用脚手架，因为这样可以快速搭建起项目。但是弊端也是有的，比如不太容易理解项目为什么要这样搭建，也可能搞不清楚各部分代码是如何配合的，
    1.便于扩展。（前端脚手架配置复杂，有时改造第三方的框架，先要非常熟悉框架，当这个框架比较复杂时，）这样的工作量和难度就大大加大了而自实现的框架(库)则可以根据需要任意扩展，可以根据需求制定对应的规范和API。
    2.便于统一定制开发规范。（模块化，组件化，规范化，自动化）
    3.减少升级带来的风险。（配置是自己从0到一配置的，升级起来坑很少）
    4.更好的维护框架以及配置。
    5.可以跟好的学习框架知识（raect，router，webpack，ts，babel，等）
    前端工程化不单单包括前端的模块化、组件化、规范化，自动化，前端性能优化，前端性能监控等一系列知识
    前端工程化不是某个具体的工具，而是对项目的整体架构与整体规划，使开发者能在未来可判时间内动态规划发展走向，以提升整个项目对用户的服务周期。
        🔥🔥🔥包括但不限于脚手架、组件库、工具库、私有仓库、接口系统、文档系统、监控系统、CICD、可移植容器
    🍒前端工程化是其主要目的为了提高效率和降低成本，即提高开发过程中的开发效率，减少不必要的重复工作时间等 方便管理、专注开发、快速迭代等
        1.模块化 （作用： 隔离作用域，提供复用性，提高可维护性，解决命名冲突，抽离公共代码）
           JS模块化方案 AMD/CMD/CommonJS/UMD/ES6 Module等等 CSS模块化方案 css modules
        2.组件化 （UI组件，容器逻辑组件，权限组件，通用组建） （模块化只是在文件层面上，对代码和资源的拆分；组件化是在设计层面上，对于UI的拆分）
        3.规范化     在项目规划初期制定的好坏对于后期的开发有一定影响。包括的规范有
          》目录结构的制定，编码规范化，前后端接口规范，文档规范化，git分支管理，组件库文档管理，Commit提交描述规范，codeReview规范
        4.自动化 （自动化构建、自动化部署、自动化测试等等CICD,持续集成、）
        5.前端性能优化（代码优化，构建工具优化，网络层面优化），性能监控（用户数据监控，页面性能监，控异常监控，）
...
🔥🔥🔥 🔥🔥🔥🔥调研ElasticSearch搜索与分析的技术，对接ElasticSearch查询，封装查询语法，提高查询效率。
    Elasticsearch是一种分布式的海量数据搜索与分析的技术，用于对海量的数据进行存储，搜索和分析，大量数据
...
🔥🔥🔥 🔥🔥🔥🔥优化项目质量，减少项目编译时间，构建时间，减小包大小，提高网站的性能。
    项目webpack搭建的，结合raect代码优化。代码层面，构建工具层面，网络层面三分方面分析
🔥🔥🔥🔥 🔥🔥🔥🔥带领团队参加需求评审，参与需求开发，指定排期，推动项目按时上线，提高项目交付质量和效率。
    🌰目标
    1.把控需求 （调研需求）
    2.技术方案设计 （需求指导设计，设计指导开发。）
    3.开发 （作为技术负责人）应该去写一些通用能力，核心功能，底层逻辑的代码。其他比较简单的业务代码，可以交给项目成员来完成。
    4.监督代码质量
        技术负责人，可能会带领好多人一起编写代码，但他要把控整个项目的代码质量。例如：
            制定代码规范  定期组织代码审核  CI 时使用自动化单元测试
    5.跟踪进度
...
🔥🔥🔥🔥 🔥🔥🔥🔥推动项目CI/CD构建前端自动化部署项目。优化了构建流程，提升了部署效率。
    ...
    master ：保护分支，对应的就是生产环境的分支dev：开发分支&脏分支，对应的是大家共用的开发环境release：保护分支，所有开发完成的分支会申请合并到release分支，提供给测试人员测试
