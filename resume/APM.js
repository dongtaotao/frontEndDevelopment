1.项目背景
    ...
    总之无法统一收集与管理。无法统一的对比app的质量。
    为了防止数据泄漏以及安全，这时候成立了性能监控分析平台，让所有的业务线对接我们的平台，性能数据统一的投递到到我们的网站展示分析。

    为了统一规范，架构组提供了很多监控性能的SDK
    提供了关键性能指标的SDK： 奔溃，卡顿，内存分析，冻帧分析，ARN分析（用户无响应）
    页面性能数据SDK（app启动时间、页面打开时间、埋点trace时间分析），
    网络监控数据SDK（http请求性能、dns、webview请求），
    热补数据SDK（耗时、成功率、到达率），
    业务异常数据SDK（异常栈、业务模块、附加数据）

本次性能监控分析平台就是为了收集所有app的性能数据做，多维度的分析可是化展示的，同时对接了公司项目管理系统，可以跟好的解决性能的bug
以及打通公司报警订阅功能，实时监控性能数据，发送email给指定的负责人。

收益结果
通过项目上线，目前公司80%的业务方对接了我们的监控系统
给线上app的质量带来了，
我们通过一个定制的计算公司，计算出每个app线上的质量 崩溃30% + 卡顿20% + 内存泄漏10% + 异常*20%
崩溃率20%的提升
卡顿率10%
...
🔥🔥🔥🔥🔥🔥🔥🔥负责技术选型，项目框架搭建，集成框架React全家桶，让项目更加模块化，组件化，规范化，自动化。
    ...
    🍒1.工程化是一种思想，而不是某种技术。其主要目的为了提高效率和降低成本，即提高开发过程中的开发效率，减少不必要的重复工作时间等
        ...
        在项目规划初期制定的好坏对于后期的开发有一定影响。包括的规范有
            ...
            3.新同事可以快速接收
            4.提高复用率
        4.自动化 （例如持续集成、自动化构建、自动化部署、自动化测试等等）
...
🔥🔥🔥 🔥🔥🔥🔥调研可视化图表，对图表进行二次封装成组建库,提升组件可维护性，扩展性，复用率。
    图表组建库（recharts） React搭配好用的图表模块Recharts
...
🔥🔥🔥🔥 🔥🔥🔥🔥封装统计SDK(数据监控，性能监控，异常监控)，保证项目的数据采集以及监控项目质量。
    ...
    其中需要注意的是我们必须使用img等来进行请求，因为img标签不会跨域且兼容性较好。
