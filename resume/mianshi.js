...
负责项目整体技术架构以及研发工作，比如整体框架搭建，技术方案制定，性能优化，制定开发规范，推动前端工程化等
...
1.项目背景
  ...
  本次性能监控分析平台就是为了收集所有业务端app的性能数据做，多维度的分析可是化展示的,接入了公司项目管理系统，最总bug以及解决bug
  添加报警订阅功能，实时监控性能数据，发送email给指定的负责人，或者聊天系统，以及公众号。
2.项目目的： 为线上app质量提供实时，大数据，全覆盖，多维度监控和报警功能。（进行多维度分析）
...
🔥🔥🔥 🔥🔥🔥🔥调研ElasticSearch搜索与分析的技术，对接ElasticSearch查询，封装查询语法，提高查询效率。
    Elasticsearch是一种分布式的海量数据搜索与分析的技术，用于对海量的数据进行近实时的数据分析，分布式执行数据分析，存储，搜索和分析
🔥🔥🔥 🔥🔥🔥🔥调研可视化图表，对图表进行二次封装成组建库,提升组件可维护性，扩展性，复用率。 echarts
    图表组建库（recharts）React 和 D3 构建的图表库    利用 React 的特性，重新定义了图表的配置和组合方式，大大地提高了图表自定义样式的灵活度
...
 🔥🔥🔥🔥 🔥🔥🔥🔥 推动项目code-review，编写代码风格指南，代码开发规范，保证代码的统一和健壮以及可维护性，提升开发效率。   
    🌰code review ================================================
    1.代码规范（eslint不能全检查，如变量命名，代码语义）2.重复代码需要抽离，复用
    3.单个函数内容过长，需要拆分
    4.算法复杂度是否可用？是否可继续优化
    5.是否有安全漏洞
...
2.用户是否有权限控制页面逻辑。比如页面中的报警订阅装置只有leader才可以触发，还有就是分配bug，pms等
   ...
   网页对url有长度限制，这时候我们只能存储关键的数据，比如奔溃页面的关键数据
