...
用户访问app1系统，app1系统是需要登录的，但用户现在没有登录。重定向到sso认证中心，并将自己的地址作为参数。
...
认证中心重定向回app系统，并把Token携带过去给app1，重定向的地址如：
www.java3y.com?token=xxxxxxx
SSO系统登录完成后会生成一个ST（Service Ticket） ，然后跳转到app1系统，同时将ST作为参数传递给app1系统。

app1系统拿到ST后，从后台向SSO发送请求，验证ST是否有效。

验证通过后，app1系统将登录状态写入session并设置app域下的Cookie。注意，此处的 cookie 和 session 保存的是用户在 app1 系统的登录状态，和 CAS 无关。
...
作者：爱做梦的小子
链接：https://juejin.cn/post/7090926280127807501
